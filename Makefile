...
NETWORK_ARGS := --rpc-url $(SCROLL_SEPOLIA_RPC_URL) --account $(ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
else
ifeq ($(findstring --network scroll,$(ARGS)),--network scroll)
NETWORK_ARGS := --rpc-url $(SCROLL_RPC_URL) --account $(ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
else
ifeq ($(findstring --network celo,$(ARGS)),--network celo)
NETWORK_ARGS := --rpc-url $(CELO_RPC_URL) --account $(ACCOUNT) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
else
...
deploy-sepolia:
	@forge script script/RebalancingPosition.s.sol:DeployRebalancingPosition --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
